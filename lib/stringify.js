...
/*
 * Expression for an escape expression.
 *
 * @example
 *   var expr = /^\\([!@#$%^&])/;
 *   expr.source.match(ESCAPE_EXPRESSION)[1] // '[!@#$%^&]'
 */
var ESCAPE_EXPRESSION = /^\^\\{2}\((?:[^|[]*\|)?(.+)\)$/;
...
/**
 * Factory to encode text nodes.
 * Combines `encodeEntitiesFactory` with `encodeEscapesFactory`.
 *
 * @param {Object} options - Compiler options, passed to
 *   `encodeEscapesFactory`.
 * @param {string} entitiesType - Either `'true'`, `'false'`, or
 *   `numbers`, passed to `encodeEntitiesFactory`.
 * @param {File} file - Currently compiled virtual file.
 * @return {function(string): string} - Function which
 *   takes a value and returns its encoded version.
 */
function encodeFactory(options, entitiesType, file) {
    var encodeEntities = encodeEntitiesFactory(entitiesType, file);
    var encodeEscapes = encodeEscapesFactory(options, file);

    /**
     * Encode value by passing it through `encodeEscapes`
     * and `encodeEntities`.  Note that the order is important here.
     *
     * @param {string} value
     * @param {Object} node
     * @return {string}
     */
    return function encode (value, node) {
        return encodeEntities(encodeEscapes(value, node), node);
    };
}
 ...
 *     encodeEntitiesFactory('true')('\x01') // throws
 *
 ...
 *   var file = new File();
 *
 *   var encode = encodeEntitiesFactory('false', file);
 *   encode('AT&T') // 'AT&T'
 *
 *   encode = encodeEntitiesFactory('true', file);
 *   encode('AT&T') // 'AT&amp;T'
 *
 *   encode = encodeEntitiesFactory('numbers', file);
 *   encode('AT&T') // 'ATT&#x26;T'
 *
 ...
 *   takes a value and returns its encoded version.
 */
function encodeEntitiesFactory(type, file) {
    var options = {};
    var fn;
     ...
     *   invalid HTML.
     */
    function encodeEntities(value, node) {
        try {
 ...
 *   takes a value and returns its encoded version.
 */
function encodeEscapesFactory(options) {
    var escapeRule = expressions[options.commonmark && 'commonmark' ||
                                 options.gfm && 'gfm' ||
                                 'rules'].escape;
    var escapes = RegExp(escapeRule.source.match(ESCAPE_EXPRESSION)[1], 'g');

    /**
     * Encode value by escaping punctuation characters.
     *
     * @param {string} value
     * @return {string}
     */
    return function encodeEscapes (value) {
        return value.replace(escapes, '\\$&');
    };
}
...
compilerPrototype.setOptions = function (options) {
    ...
    var entitiesType = String(options.entities);
    self.encodeEntities = encodeEntitiesFactory(entitiesType, self.file);
    self.encode = encodeFactory(options, entitiesType, self.file);
...
compilerPrototype.text = function (node, parent) {
    var value = this.encode(node.value, node);

    if (
        value[value.length - 1] == BACKSLASH &&
        parent &&
        parent.type == 'linkReference' &&
        parent.referenceType == 'shortcut'
    ) {
        // Don't escape trailing backslash in shortcut links.
...
compilerPrototype.code = function (node, parent) {
    ...
    var language = self.encodeEntities(node.lang || EMPTY, node);
    var fence;
...
compilerPrototype.link = function (node) {
    ...
    if (
        ...
        return encloseURI(self.encodeEntities(node.href), true);
    }
...
compilerPrototype.imageReference = function (node) {
    var alt = this.encodeEntities(node.alt, node);
...
compilerPrototype.image = function (node) {
    ...
    if (node.title) {
        url += SPACE + encloseTitle(this.encodeEntities(node.title, node));
    }

    value = EXCLAMATION_MARK +
        SQUARE_BRACKET_OPEN + this.encodeEntities(node.alt || EMPTY, node) +
        SQUARE_BRACKET_CLOSE;
