...
function encodeFactory(options, entitiesType, file) {
     ...
     * @return {string}
     */
    return function encode (value, node, parent) {
        return encodeEntities(encodeEscapes(value, node, parent),
                              node, parent);
    };
}
...
function encodeEscapesFactory(options) {
     ...
     * @return {string}
     */
    return function encodeEscapes (value, node, parent) {
        var encoded = value
            .replace(/[\\`*\[_]/g, '\\$&')
            .replace(/(\])(\()/g, '$1\\$2')
            .replace(/(\n\s*)([->])/g, '$1\\$2');
        ...
        if (node.type == 'text' && parent) {
            ...
            {
                encoded = encoded.replace(match[1], '\\' + match[1]);
            }

            if (value[0] == '(' && previous &&
                previous.type == 'text' && previous.value.slice(-1) == ']')
            {
                encoded = '\\' + encoded;
            }
...
compilerPrototype.text = function (node, parent) {
    var value = this.encode(node.value, node, parent);
