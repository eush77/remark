...
MERGEABLE_NODES.text = function (prev, node) {
    if (unmergeable(prev) || unmergeable(node)) {
        return node;
    } else {
    ...
    /**
     * Check whether node is mergeable with adjacent nodes.
     *
     * @param {Object} node
     * @return {boolean} - False if mergeable, true if not.
     */
    function unmergeable(node) {
        if (!node.position) {
            return true;
        }
        ...
        // Nodes with such a position info cannot be merged.
