...
function tokenizeCode(eat, value, silent) {
    ...
    if (content) {
        ...
        return eat(subvalue)(self.renderCodeBlock(content, null, eat));
    }
}
...
function tokenizeFences(eat, value, silent) {
    ...
    /** Eat zero or more space / tab characters. */
    function eatOptionalSpacing() {
        while (++index < length) {
            character = value.charAt(index);
    ...
    return eat(subvalue)(self.renderCodeBlock(exdentedContent, flag, eat));
}
...
function tokenizeDefinition(eat, value, silent) {
    ...
    if (character === C_LT) {
        test = isEnclosedURLCharacter;
        subvalue += character;
        queue = EMPTY;
    } else {
        test = isUnclosedURLCharacter;
        queue = character;
    }
    ...
    if (!character || character === C_NEWLINE) {
        ...
        return eat(subvalue)({
            ...
            'link': decode(self.descape(url), eat)
        });
    }
}
...
function tokenizeText(eat, value, silent) {
    ...
    return eat(subvalue)(self.renderRaw(T_TEXT, subvalue));
}
...
function tokenizeAutoLink(eat, value, silent) {
    ...
    node = eat(subvalue)(
        self.renderLink(true, link, content, null, now, eat)
    );
...
function tokenizeURL(eat, value, silent) {
    ...
    return eat(subvalue)(
        self.renderLink(true, subvalue, content, null, now, eat)
    );
}
...
function tokenizeLink(eat, value, silent) {
    ...
    return eat(subvalue)(
        self.renderLink(!isImage, self.descape(url), content, title, now, eat)
    );
}
...
function Parser(file, options, processor) {
    ...
    self.descape = descapeFactory(self, T_ESCAPE);
...
function tokenizeFactory(type) {
    ...
    function tokenize(value, location) {
        ...
        add = function (node, parent) {
            ...
            if (type === INLINE && node.type === T_TEXT) {
